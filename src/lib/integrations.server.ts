...
async function buildPayload(supabaseAdmin: any, orderId: string) {
  ...
  return {
    ...
    items: (items ?? []).map((i: any) => ({
      ...
      units_per_pack: Math.max(1, Number(i.metadata?.units_per_pack ?? 1)),
    })),
    addresses: addresses ?? [],
  ...
  const pedido: Record<string, unknown> = {
    ...
    itens: p.items.map((i: any) => ({
      item: {
        ...
        unidade: "UN",
        quantidade: i.quantity * i.units_per_pack,
        valor_unitario: (i.unit_price / i.units_per_pack).toFixed(4),
      },
    })),
